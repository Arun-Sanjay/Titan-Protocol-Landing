...
async function readParams(request: Request): Promise<CallbackParams> {
  if (request.method === "POST") {
    ...
    return {
      ...
      paymentLinkId:
        formData.get("razorpay_payment_link_id")?.toString() ?? null,
      paymentLinkReferenceId:
        formData.get("razorpay_payment_link_reference_id")?.toString() ?? "",
      paymentLinkStatus:
        formData.get("razorpay_payment_link_status")?.toString() ?? null,
...
async function handleCallback(request: Request) {
  ...
  if (!paymentId || !signature) {
    return NextResponse.redirect(
      new URL("/checkout?payment=missing", request.url),
    );
  }
  ...
  if (!isVerified) {
    return NextResponse.redirect(
      new URL("/checkout?payment=invalid", request.url),
    );
  }

  // Verified — drop the access cookie and send them to /download.
